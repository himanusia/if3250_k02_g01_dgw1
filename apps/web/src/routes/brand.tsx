  ...
  return (
    <div className="h-full overflow-y-auto bg-gradient-to-b from-background via-[#fff6f8] to-background">
      <div className="container mx-auto grid gap-6 px-4 py-6 lg:py-8">
        <section className="overflow-hidden rounded-[2rem] border border-[#b43c39]/15 bg-white shadow-[0_24px_80px_rgba(123,32,76,0.12)]">
        ...
        <section className="grid gap-4">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            ...
            <Link
              to="/campaigns"
              className="inline-flex w-fit items-center justify-center rounded-full bg-[#7B204C] px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-[#5f183a]"
            >
              Kelola campaign
            </Link>
          </div>
          ...
          {campaignsQuery.isLoading ? (
            <div className="rounded-3xl border border-dashed border-[#b43c39]/30 bg-white/70 p-8 text-center text-muted-foreground">
              Memuat brand...
            </div>
            ...
            <div className="rounded-3xl border border-dashed border-[#b43c39]/30 bg-white/80 p-8 text-center">
              ...
              <Link
                to="/campaigns"
                className="mt-5 inline-flex items-center justify-center rounded-full bg-[#B43C39] px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-[#8f2e2c]"
              >
                Buat campaign
              </Link>
            </div>
...
function MetricCard({ description, label, value }: { description: string; label: string; value: string }) {
  return (
    <div className="rounded-3xl border border-[#b43c39]/15 bg-white p-5 shadow-sm">
...
function BrandCard({ brand }: { brand: BrandSummary }) {
  ...
  return (
    <article className="rounded-3xl border border-[#b43c39]/15 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-[0_18px_50px_rgba(123,32,76,0.12)]">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="rounded-2xl bg-[#7B204C]/10 px-4 py-3 text-right">
      ...
      <div className="mt-5 flex flex-wrap gap-2">
        {brand.platforms.length ? (
          brand.platforms.map((platform) => (
            <span key={platform} className="rounded-full bg-[#fff3d8] px-3 py-1 text-xs font-semibold text-[#7B204C]">
              {platform}
            </span>
          ))
        ) : (
      ...
      <div className="mt-5 space-y-2">
        {latestCampaigns.map((campaign) => (
          <Link
            key={campaign.id}
            to="/campaigns"
            className="block rounded-2xl border border-border/70 px-4 py-3 transition hover:border-[#B43C39]/40 hover:bg-[#fff6f8]"
          >
